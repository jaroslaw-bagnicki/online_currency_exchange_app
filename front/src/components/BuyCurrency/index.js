...
export class BuyCurrency extends Component {
  ...
  handleChange = (quantity, price, unit) => {
    if (quantity < 0) {
    ...
    } else if (quantity > (this.props.wallet.balance / price)) {
      this.props.orderError('Transaction amount can not be greater than the balance!');
      this.setState({
        quantity: Math.floor(this.props.wallet.balance / price),
        price
      });
    } else {
  ...
  render() {
    const { isSignIn, rates, wallet: { balance }, order: {isProcessing, isConfirmed, error, message } } = this.props;
    const code = this.props.match.params.code;
    const currency = rates.items.find(item => item.code === code);
    const volume = this.state.quantity * currency.unit;
    const amount = (this.state.quantity * currency.sellPrice).toFixed(2);
        ...
        <div className="row">
          <div className="col s12 m8 offset-m2 l6 offset-l3">
            <div className="section">
              <h5 className="grey-text text-darken-3">Buy {currency.name} currency</h5>
              ...
              <div className="input-field">
                ...
                <input type="number" id="quantity" value={this.state.quantity} onChange={(e) => this.handleChange(Number.parseFloat(e.target.value), currency.sellPrice, currency.unit)} />
              </div>
              <div className="input-field">
              ...
              <div className="input-field">
                <label className="active" htmlFor="balance">Account balance</label>
                <input type="number" id="balance" value={balance || 0} disabled />
              </div>
